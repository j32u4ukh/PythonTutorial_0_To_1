...
為例：
私たちは同じパッケージから複数の関数をインポートしているとき、複数の方法があります。
log2 と log10 を例として説明します。

＝＝＝＝＝
1. import 共同套件，使用時前面都加上套件名稱，優點是可以清楚知道這個函式來自哪個套件，
但缺點就是程式碼會變得比較長。
1. 共通(きょうつう)するパッケージをインポートして、使うとき、関数の前にこのパッケージ
の名前を続いて。長所(ちょうしょ)はこの関数がどのパッケージからことをはっきり分かる、
短所(たんしょ)はコードの長さが長くなる。
import math
math.log2(...)
math.log10(...)
...
，不是個好的做法。
2. 一行(いちぎょう)に一つ関数をインポートして、長所は関数が要らないとき、すぐに
削除(さくじょ)できます。短所は重複(ちょうふく)コンテンツが多くすぎて、いい方法じゃない
と思います。
from math import log2
...
數量一多，這行引用也會變得過長。
3. 読点(とうてん)で同じパッケージの関数に分かれる。長所はコンテンツが重複しない、でも
短所はインポートされるの数量が多くになると、コードの長さも長くなる。
from math import log2, log10

＝＝＝＝＝
4. 用逗號分隔同個套件下的不同函式，搭配括號來換行，優點是排除了前一種方法的缺點。
4. 丸括弧(まるかっこ)で関数を取り囲んで(とりかこんで)、これなら前の方法の欠点がいなく
なる。
from math import (
          log2, 
          log10
...
因此把它的說明放到了最後。
最初の時言った、パッケージを使いたいなら、設置(せっち)しなければならない。だが、ここで
インストールではない、 cmd に実行するです。だから、最後のところに説明をします。

＝＝＝＝＝
cmd 可以直接在電腦中開啟搜尋，並輸入 cmd 後便可開啟。一般會建議初學者透過 Anaconda 
來安裝 Python ，因此要安裝套件時，需要先進入所謂的'虛擬環境'，然後輸入 
pip install XXX 來進行安裝。
コンピューターで検索(けんさく)機能(きのう)開けて、 cmd を入れって、 cmd が開けられる。
一般的(いっぱんてき)に言えば、初心者(しょしんしゃ)に” Anaconda で設置して方がいい”を
提案(ていあん)する。だから、設置しているとき、先ずは"仮想(かそう)環境(かんきょう)"に
入って(はいって)、pip install XXX を入れて(いれて)設置する。

＝＝＝＝＝
而 Python 本身也有附帶一些'套件'，可以讓你省去安裝這個步驟，'套件' math 就是其中
一個例子。
Python は少しパッケージを添付(てんぷ)して、設置が要らない。例えば： math というの
パッケージ。

＝＝＝＝＝
我知道上面的說明對'初學者'可能有看沒有懂，在這個檔案在的資料夾當中，還有一個叫做 
pip_learning.md 的檔案，裡面有圖片說明應該會比較清楚。
上の説明は初心者にまだ難しい。このファイルが入っているフォルダー内は pip_learning.md 
というのファイルがあります、写真を連れて、理解やすいと思います。

＝＝＝＝＝
當整份教程完成後，我也會將一些過程中使用到但沒有說明的部分一併補上的。
全てのチュートリアルは完成したあと、使ったけど説明されない部分を追記(ついき)します。
"""