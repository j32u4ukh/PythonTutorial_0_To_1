...
変数は他のタイプもありますが、これから使われる変数を勉強が十分だ。

首先是整數 int，如：1, 2, 3, 9527, ....。
先ずは整数(せいすう) int、例えば：1, 2, 3, 9527, ....。
"""
print(1)
print(2)
print(3)
...
浮動小数点数(ふどうしょうすうてんすう) float、つまり、小数点を付けているの数字だ。
"""
print(2.71828)
print(3.14159)
print(9.0)
...
この前言った整数と紐などは変数に交換できます。再利用に便利。

＝＝＝＝＝
這個過程叫做"賦值"，將"右邊"的值給予"左邊"的變數，之後還會提到，請記得"賦值"的意思。
このプロセスは代入と申します、左側の変数に右側の数値(すうち)を代入する。これからも何回
目に言及されだから、代入の意味を覚えてくださ。
"""
t0 = 1
t1 = "字串"
t2 = 2.71828
