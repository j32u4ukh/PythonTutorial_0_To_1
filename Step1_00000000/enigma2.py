...
class Rotor(Pipeline):
    def __init__(self, _pointer, _items, _mode):
        super().__init__()
        self.addOriginItems(_items)
    ...
    def forwardSwap(self, _input):
        ...
        _swap_index = _input
        if self.mode is Mode.Encode:
            _swap_index += self.pointer
    ...
    def backwardSwap(self, _input):
        ...
        if self.mode is Mode.Decode:
            _output -= self.pointer
...
class Enigma:        
    def __init__(self, _items, _mode):
        self.items = _items
        self.rotors = []
    ...
    def swap(self, _input):
        if self.mode is Mode.Encode:
            return self.encode(_input)
        elif self.mode is Mode.Decode:
            return self.decode(_input)
...
def rotorTest():
    ...
    _rotor = Rotor(0, _items, Mode.Encode)
...
def enigmaTest():
    ...
    _enigma = Enigma(_items, Mode.Encode)
    _rotor = Rotor(0, _items, Mode.Encode)
    _enigma.add(_rotor)
    _enigma.compile_()
    print("origin:   ", _rotor.origin_sequence)
...
def enigmaTest2():
    ...
    _enigma.mode = Mode.Decode
    _enigma.rotors[0].pointer = 0
    _enigma.rotors[0].mode = Mode.Decode
